...
// }
  ...
  crashWith() {
      return (this.bottom() - (this.height * .75)) > myGameArea.canvas.height
  }
}
...
window.addEventListener("keyup",  (event) => {
  ...
  player.speedX = 0;
  if(!player.followingPlatform) player.speedY = 6; // allows for decending
}, false);

function checkIfStopFalling() {
  player.followingPlatform = myObstacles.some(function (obstacle) {
    return player.stopFallingAndFollow(obstacle);
  });
}

function checkGameOver() {
  const crashedByFalling = player.crashWith();
  if (crashedByFalling) myGameArea.stop();
}
